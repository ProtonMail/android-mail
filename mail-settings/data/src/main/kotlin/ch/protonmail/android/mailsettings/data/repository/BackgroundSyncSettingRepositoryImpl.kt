...
class BackgroundSyncSettingRepositoryImpl @Inject constructor(
    private val dataStoreProvider: MailSettingsDataStoreProvider
) : BackgroundSyncSettingRepository {

    private val backgroundSyncPreferenceKey = booleanPreferencesKey("allowBackgroundSyncPrefKey")

    override fun observe(): Flow<Either<PreferencesError, BackgroundSyncPreference>> =
        dataStoreProvider.backgroundSyncDataStore.safeData.map { preferences ->
            preferences.map {
                val isEnabled = it[backgroundSyncPreferenceKey] ?: DefaultValue
                BackgroundSyncPreference(isEnabled)
            }
        }
    ...
    override suspend fun update(value: Boolean): Either<PreferencesError, Unit> {
        return dataStoreProvider.backgroundSyncDataStore.safeEdit { preferences ->
            preferences[backgroundSyncPreferenceKey] = value
        }.map { Unit.right() }
    }
