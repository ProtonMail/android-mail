...
class AutoLockRepositoryImplTest {
    ...
    private lateinit var autoLockRepository: AutoLockRepositoryImpl
    ...
    fun `returns false when no preference is stored locally`() = runTest {
        ...
        // When
        autoLockRepository.observe().test {
            // Then
            assertEquals(AutoLockPreference(false), awaitItem())
            awaitComplete()
        }
    }
    ...
    fun `returns locally stored preference from data store when available`() = runTest {
        // Given
        coEvery { preferences[booleanPreferencesKey("hasAutoLockPrefKey")] } returns true
        // When
        autoLockRepository.observe().test {
            // Then
            assertEquals(AutoLockPreference(true), awaitItem())
            awaitComplete()
        }
    }
}
