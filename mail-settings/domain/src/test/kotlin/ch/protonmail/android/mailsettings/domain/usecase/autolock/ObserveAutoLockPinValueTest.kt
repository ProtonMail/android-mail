...
internal class ObserveAutoLockPinValueTest {

    private val autoLockRepository = mockk<AutoLockRepository>()
    private val observeAutoLockPinValue = ObserveAutoLockPinValue(autoLockRepository)

    @Test
    fun `should propagate the value when the repo returns a success`() = runTest {
        // Given
        val expectedValue = AutoLockPin("0303").right()
        every { autoLockRepository.observeAutoLockPin() } returns flowOf(expectedValue)

        // When
        val actual = observeAutoLockPinValue().firstOrNull()
