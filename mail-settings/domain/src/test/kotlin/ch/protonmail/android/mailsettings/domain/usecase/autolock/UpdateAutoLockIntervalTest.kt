...
internal class UpdateAutoLockIntervalTest {

    private val autoLockRepository = mockk<AutoLockRepository>()
    private val updateAutoLockInterval = UpdateAutoLockInterval(autoLockRepository)

    @Test
    fun `should propagate the value when the repo returns a success`() = runTest {
        // Given
        val newPreference = AutoLockInterval.FifteenMinutes
        val expectedValue = Unit.right()
        coEvery { autoLockRepository.updateAutoLockInterval(newPreference) } returns expectedValue

        // When
        val actual = updateAutoLockInterval(newPreference)
