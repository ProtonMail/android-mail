...
internal class ObserveAutoLockEnabledTest {

    private val autoLockRepository = mockk<AutoLockRepository>()
    private val observeAutoLockEnabled = ObserveAutoLockEnabled(autoLockRepository)

    @Test
    fun `should propagate the value when the repo returns a success`() = runTest {
        // Given
        val expectedValue = AutoLockPreference(true).right()
        every { autoLockRepository.observeAutoLockEnabledValue() } returns flowOf(expectedValue)

        // When
        val actual = observeAutoLockEnabled().firstOrNull()
