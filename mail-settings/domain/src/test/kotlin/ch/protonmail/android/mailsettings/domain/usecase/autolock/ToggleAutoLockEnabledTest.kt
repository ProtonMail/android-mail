...
internal class ToggleAutoLockEnabledTest {

    private val autoLockRepository = mockk<AutoLockRepository>()
    private val toggleAutoLockEnabled = ToggleAutoLockEnabled(autoLockRepository)

    @Test
    fun `should propagate the value when the repo returns a success`() = runTest {
        // Given
        val newPreference = AutoLockPreference(true)
        val expectedValue = Unit.right()
        coEvery { autoLockRepository.updateAutoLockEnabledValue(newPreference) } returns expectedValue

        // When
        val actual = toggleAutoLockEnabled(newPreference.isEnabled)
