...
internal class UpdateLastForegroundMillisTest {

    private val autoLockRepository = mockk<AutoLockRepository>()
    private val updateLastForegroundMillis = UpdateLastForegroundMillis(autoLockRepository)

    @Test
    fun `should propagate the value when the repo returns a success`() = runTest {
        // Given
        val newValue = AutoLockLastForegroundMillis(123L)
        val expectedValue = Unit.right()
        coEvery { autoLockRepository.updateLastForegroundMillis(newValue) } returns expectedValue

        // When
        val actual = updateLastForegroundMillis(newValue.value)
