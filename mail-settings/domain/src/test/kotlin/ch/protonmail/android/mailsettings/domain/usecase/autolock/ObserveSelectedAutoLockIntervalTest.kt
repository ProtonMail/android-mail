...
internal class ObserveSelectedAutoLockIntervalTest {

    private val autoLockRepository = mockk<AutoLockRepository>()
    private val observeSelectedAutoLockInterval = ObserveSelectedAutoLockInterval(autoLockRepository)

    @Test
    fun `should propagate the value when the repo returns a success`() = runTest {
        // Given
        val expectedValue = AutoLockInterval.OneHour.right()
        every { autoLockRepository.observeAutoLockInterval() } returns flowOf(expectedValue)

        // When
        val actual = observeSelectedAutoLockInterval().firstOrNull()
