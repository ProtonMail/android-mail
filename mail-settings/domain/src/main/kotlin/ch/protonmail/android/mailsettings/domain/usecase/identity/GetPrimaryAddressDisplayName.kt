...
class GetPrimaryAddressDisplayName @Inject constructor(
    private val getPrimaryAddress: GetPrimaryAddress
) {

    suspend operator fun invoke(userId: UserId): Either<DataError, DisplayName> = either {
        val address = getPrimaryAddress(userId).getOrNull() ?: raise(DataError.AddressNotFound)
