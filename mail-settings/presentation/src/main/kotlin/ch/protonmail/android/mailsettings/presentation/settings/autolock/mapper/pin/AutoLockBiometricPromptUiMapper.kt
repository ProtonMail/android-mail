...
class AutoLockBiometricPromptUiMapper @Inject constructor() {

    fun toUiModel(state: AutoLockBiometricsState): Effect<Unit> {
        return when (state) {
            is AutoLockBiometricsState.BiometricsAvailable.BiometricsEnrolled ->
                if (state.enabled) Effect.of(Unit) else Effect.empty()
