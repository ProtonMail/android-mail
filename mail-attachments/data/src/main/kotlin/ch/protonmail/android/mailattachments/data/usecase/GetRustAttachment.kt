...
package ch.protonmail.android.mailattachments.data.usecase
...
class GetRustAttachment @Inject constructor() {
    ...
    ): Either<DataError, DecryptedAttachment> = when (val result = mailUserSession.getAttachment(attachmentId)) {
        is MailUserSessionGetAttachmentResult.Error -> result.v1.toDataError().left()
        is MailUserSessionGetAttachmentResult.Ok -> result.v1.right()
    }
}
