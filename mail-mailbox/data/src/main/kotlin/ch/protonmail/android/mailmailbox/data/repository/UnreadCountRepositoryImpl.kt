...
class UnreadCountRepositoryImpl @Inject constructor(
    ...
    override suspend fun observeUnreadCount(userId: UserId): Flow<UnreadCounters> {
        remoteDataSource.getMessageCounters(userId)
        remoteDataSource.getConversationCounters(userId)

        return flowOf(UnreadCounters(emptyList(), emptyList()))
    }
