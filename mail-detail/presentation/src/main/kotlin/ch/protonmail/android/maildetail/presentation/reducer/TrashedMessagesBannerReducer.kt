...
class TrashedMessagesBannerReducer @Inject constructor() {
    ...
    fun newStateFrom(operation: ConversationDetailOperation.AffectingTrashedMessagesBanner) = when (operation) {
        ...
        when {
            filterByLocation == Trash.labelId && messagesLabelIds.any { it.value.contains(Trash.labelId).not() } -> {
                when {
                    ...
                    else -> TrashedMessagesBannerState.Shown(
                        TrashedMessagesBannerUiModel(R.string.non_trashed_messages_banner, R.string.hide)
                    )
                }
            }
            filterByLocation != Trash.labelId && messagesLabelIds.any { it.value.contains(Trash.labelId) } -> {
                when {
                    shouldHideMessagesBasedOnTrashFilter -> TrashedMessagesBannerState.Shown(
                        ...
                        TrashedMessagesBannerUiModel(R.string.trashed_messages_banner, R.string.hide)
                    )
                }
            }
            else -> TrashedMessagesBannerState.Hidden
        }
    }
}
