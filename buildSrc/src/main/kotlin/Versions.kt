object Versions {

    object Accompanist {
        const val accompanist = "0.19.0"
    }

    object Android {
        const val material = "1.4.0"
    }

    object AndroidX {
        const val activity = "1.3.1"
        const val annotation = "1.2.0"
        const val compose = "1.0.3"
        const val hilt = "1.0.0"
        const val hiltNavigationCompose = "1.0.0-alpha03"
        const val lifecycle = "2.3.1"
        const val navigation = "2.3.5"
        const val navigationCompose = "2.4.0-alpha04"
        const val paging = "3.0.1"
        const val pagingCompose = "1.0.0-alpha12"
        const val room = "2.3.0"
        const val work = "2.6.0"
    }

    object Core {
        const val account = "1.15.2"
        const val accountManager = "1.16.1"
        const val auth = "1.18"
        const val contact = "1.18"
        const val country = "1.16.1"
        const val crypto = "1.16"
        const val data = "1.15"
        const val dataRoom = "1.16"
        const val domain = "1.18"
        const val eventManager = "1.18"
        const val humanVerification = "1.16.2"
        const val key = "1.16.1"
        const val mailSettings = "1.18"
        const val network = "1.15.7"
        const val payment = "1.17.2"
        const val plan = "1.18"
        const val presentation = "1.18.3"
        const val user = "1.18"
        const val userSettings = "1.18.1"
        const val utilKotlin = "1.15.2"
    }

    object Dagger {
        const val dagger = "2.39.1"
    }

    object Gotev {
        const val cookieStore = "1.3.5"
    }

    object Gradle {
        const val androidGradlePlugin = "7.0.3"
        const val hiltAndroidGradlePlugin = Dagger.dagger
        const val kotlinGradlePlugin = Kotlin.kotlin
        const val protonDetektPlugin = "0.4"
        const val jacocoGradlePlugin = "0.8.7"
    }

    object JaCoCo {
        const val tool = "0.8.7"
    }

    object JakeWharton {
        const val timber = "4.7.1"
    }

    object Kotlin {
        const val kotlin = "1.5.30"
    }

    object KotlinX {
        const val coroutines = "1.5.2"
        const val serializationJson = "1.3.0"
    }

    object Squareup {
        const val leakCanary = "2.7"
        const val okhttp = "4.9.2"
        const val retrofit = "2.9.0"
    }

    object Test {
        const val junit = "4.13.2"
        const val test = "1.4.0-beta02"
        const val espresso = "3.4.0"
    }

    object Sentry {
        const val sentry = "1.7.23"
    }
}
