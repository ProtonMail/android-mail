...
class GetLabels @Inject constructor(private val labelRepository: LabelRepository) {

    suspend operator fun invoke(userId: UserId, labelType: LabelType): Either<DataError, List<Label>> = Either.catch {
        labelRepository.getLabels(userId, labelType)
    }.mapLeft {
        val error = when (it) {
