...
class ReadAttachmentsFromStorageTest {
    ...
    private val readAttachmentsFromStorage = ReadAttachmentsFromStorage(attachmentRepository)
    ...
    fun `should return files if all attachments were read successfully`() = runTest {
        ...
        coEvery {
            attachmentRepository.readFileFromStorage(UserId, MessageId, AttachmentId1)
        } returns Attachment1.right()
        coEvery {
            attachmentRepository.readFileFromStorage(UserId, MessageId, AttachmentId2)
        } returns Attachment2.right()
