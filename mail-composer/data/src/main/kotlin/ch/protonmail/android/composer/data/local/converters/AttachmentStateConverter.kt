...
class AttachmentStateConverters {
    ...
    @TypeConverter
    fun fromStateToInt(state: AttachmentSyncState?): Int? = state?.value

    @TypeConverter
    fun fromIntToAttachmentState(value: Int?): AttachmentSyncState? = value?.let { AttachmentSyncState.from(it) }
