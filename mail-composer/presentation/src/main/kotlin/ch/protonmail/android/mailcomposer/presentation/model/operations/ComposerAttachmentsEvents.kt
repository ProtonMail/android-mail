...
internal sealed interface AttachmentsEvent : ComposerStateEvent {

    override fun toStateModifications(): ComposerStateModifications = ComposerStateModifications(
        attachmentsModification = when (this) {
            is OnListChanged -> AttachmentsStateModification.ListUpdated(list)
        }
    )

    data class OnListChanged(val list: List<MessageAttachment>) : AttachmentsEvent
}
