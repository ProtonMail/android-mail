...
class TrackFirstMessageSentEvent @Inject constructor(
    private val eventsRepository: EventsRepository
) {

    suspend operator fun invoke(): Either<DataError, Unit> {
        if (eventsRepository.hasSentMessageEventBeenSent()) {
            return Either.Right(Unit)
        }

        return eventsRepository.sendEvent(AppEvent.MessageSent).also {
            if (it.isRight()) {
                eventsRepository.markSentMessageEventSent()
            }
        }
    }
}
