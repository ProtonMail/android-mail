/*
 * Copyright (c) 2026 Proton Technologies AG
 * This file is part of Proton Technologies AG and Proton Mail.
 *
 * Proton Mail is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Proton Mail is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Proton Mail. If not, see <https://www.gnu.org/licenses/>.
 */

package ch.protonmail.android.mailtelemetry.data.local

import ch.protonmail.android.mailcommon.data.mapper.LocalGeneralDimensions
import ch.protonmail.android.mailcommon.data.mapper.LocalPlanSpecificDimensions
import ch.protonmail.android.mailsession.domain.wrapper.MailUserSessionWrapper
import uniffi.mail_uniffi.recordUpgradeAttempt
import uniffi.mail_uniffi.recordUpgradeCancelledByUser
import uniffi.mail_uniffi.recordUpgradeError
import uniffi.mail_uniffi.recordUpgradeSuccess
import uniffi.mail_uniffi.recordUpsellButtonTapped
import javax.inject.Inject

class UpsellingTelemetryDataSourceImpl @Inject constructor() : UpsellingTelemetryDataSource {

    override suspend fun recordUpgradeAttempt(
        mailUserSession: MailUserSessionWrapper,
        general: LocalGeneralDimensions,
        planSpecific: LocalPlanSpecificDimensions
    ) {
        recordUpgradeAttempt(
            userSession = mailUserSession.getRustUserSession(),
            general = general,
            planSpecific = planSpecific
        )
    }

    override suspend fun recordUpgradeCancelledByUser(
        mailUserSession: MailUserSessionWrapper,
        general: LocalGeneralDimensions,
        planSpecific: LocalPlanSpecificDimensions
    ) {
        recordUpgradeCancelledByUser(
            userSession = mailUserSession.getRustUserSession(),
            general = general,
            planSpecific = planSpecific
        )
    }

    override suspend fun recordUpgradeError(
        mailUserSession: MailUserSessionWrapper,
        general: LocalGeneralDimensions,
        planSpecific: LocalPlanSpecificDimensions
    ) {
        recordUpgradeError(
            userSession = mailUserSession.getRustUserSession(),
            general = general,
            planSpecific = planSpecific
        )
    }

    override suspend fun recordUpgradeSuccess(
        mailUserSession: MailUserSessionWrapper,
        general: LocalGeneralDimensions,
        planSpecific: LocalPlanSpecificDimensions
    ) {
        recordUpgradeSuccess(
            userSession = mailUserSession.getRustUserSession(),
            general = general,
            planSpecific = planSpecific
        )
    }

    override suspend fun recordUpsellButtonTapped(
        mailUserSession: MailUserSessionWrapper,
        general: LocalGeneralDimensions
    ) {
        recordUpsellButtonTapped(
            userSession = mailUserSession.getRustUserSession(),
            general = general
        )
    }
}
