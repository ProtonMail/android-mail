...
class KeyChainLocalDataSourceImpl @Inject constructor(
    ...
    override suspend fun save(key: OsKeyChainEntryKind, value: EncryptedString): Either<PreferencesError, Unit> {
        ...
        return dataStoreProvider.keyChainDataStore.safeEdit { mutablePreferences ->
            mutablePreferences[preferenceKey] = value
        }.map { }
    }

    override suspend fun remove(key: OsKeyChainEntryKind): Either<PreferencesError, Unit> {
        val preferenceKey = stringPreferencesKey(key.name)

        return dataStoreProvider.keyChainDataStore.safeEdit { it.remove(preferenceKey) }
            .map { }
    }
}
