...
class DriveSpotlightVisibilityRepositoryImpl @Inject constructor(
    private val dataStoreProvider: UpsellingDataStoreProvider
) : DriveSpotlightVisibilityRepository {

    private val preferenceKey = longPreferencesKey("driveSpotlightLastSeenPreference")

    override fun observe(): Flow<Either<PreferencesError, DriveSpotlightLastSeenPreference>> =
        dataStoreProvider.upsellingDataStore.safeData.map { preferences ->
            preferences.map {
                val value = it[preferenceKey]
                DriveSpotlightLastSeenPreference(value)
            }
        }
