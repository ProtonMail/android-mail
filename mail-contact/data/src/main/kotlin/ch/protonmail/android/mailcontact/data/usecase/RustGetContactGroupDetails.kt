...
class RustGetContactGroupDetails @Inject constructor() {
    ...
    ): Either<DataError, ContactGroupItem> =
        when (val result = contactGroupById(session.getRustUserSession(), contactGroupId)) {
            is ContactGroupByIdResult.Error -> result.v1.toDataError().left()
            is ContactGroupByIdResult.Ok -> result.v1.right()
        }
}
